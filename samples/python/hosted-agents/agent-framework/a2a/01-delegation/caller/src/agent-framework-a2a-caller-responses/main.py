...
class ToolboxAuth(httpx.Auth):
    """Injects a fresh bearer token on every toolbox MCP request."""
...
def main():
    ...
    # the sister responses/04-foundry-toolbox sample (which passes on the
    # foundry-ext deploy path). The previous async/`async with Agent(...)`
    # pattern eagerly entered the MCPStreamableHTTPTool context at startup,
    # which performs a network initialize + tools/list against the toolbox
    # MCP endpoint before the HTTP server is bound. On the foundry-ext deploy
    # path the platform probes /readiness within ~90s of container start; if
    # the MCP handshake is still in flight, /readiness never returns 200 and
    # the platform raises 424 session_not_ready on every invoke. Letting the
    # Agent enter the tool context lazily on first request avoids the
    # readiness race.
    credential = DefaultAzureCredential()

    # Create the toolbox. The toolbox is declared in agent.manifest.yaml with
    # an `a2a_preview` tool that proxies calls to a remote A2A-compatible agent
