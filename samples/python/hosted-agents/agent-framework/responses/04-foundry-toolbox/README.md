...
> [!NOTE]
> You do **not** need an existing [Microsoft Foundry](https://learn.microsoft.com/en-us/azure/ai-foundry/what-is-foundry?view=foundry) project or model deployment to get started — `azd provision` creates them for you. `azd provision` will also create the toolbox with the specified tools in `agent.manifest.yaml`. If you already have a project, see the [note below](#using-azd-recommended-for-cli-workflows) on how to target it.
...
| `TOOLBOX_NAME` | Yes | Name of the Foundry Toolbox to connect to. Declared in `agent.manifest.yaml`. |
...
mkdir toolbox-agent && cd toolbox-agent
...
# This also creates the toolbox with the specified tools in agent.manifest.yaml
azd provision
...
> `azd ai agent init -m <path-to-repo>/samples/python/hosted-agents/agent-framework/responses/04-foundry-toolbox/agent.manifest.yaml`
...
curl -sS -X POST http://localhost:8088/responses \
  ...
  -d '{"input": "What tools do you have?", "stream": false}' | jq .
```
