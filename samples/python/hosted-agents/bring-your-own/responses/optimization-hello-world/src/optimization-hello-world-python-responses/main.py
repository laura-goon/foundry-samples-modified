...
Normal operation (no optimization):
    Uses the baseline config from .agent_configs/baseline/ — behaves like a regular chat agent.

During optimization:
    The agent optimizer injects OPTIMIZATION_CONFIG / OPTIMIZATION_CANDIDATE_ID.
...
# load_config() reads them automatically; falls back to defaults if not found.
