...
"""
Browser portal for the ``livekit-server`` sample.

  Browser  <->  /ws/connect  <->  upstream invocations_ws (signaling)
                (single page)     (auth, hosted or local)
                  +
                browser  <-- LiveKit WebRTC -->  agent
                                (audio bypasses the proxy)

The signaling WS exchanges JSON messages: the browser sends
``{action: "join"}`` and the upstream replies with
``{type: "config", livekit_url, token, room, identity}``. The browser
then connects to LiveKit directly using its client SDK; audio media
flows browser <-> LiveKit <-> agent and never touches this proxy.
"""
...
DEFAULT_PORT = int(os.environ.get("PORTAL_PORT", "9529"))
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="livekit-server browser portal.")
    parser.add_argument(
