...
if [ -z "$PRODUCTION_RESOURCE" ] || [ -z "$PRODUCTION_SUBSCRIPTION" ] || [ -z "$PRODUCTION_TENANT" ]; then
    ...
    echo "  --production-resource <resource-name>       (e.g., <your-resource-name>)"
    echo "  --production-subscription <subscription-id> (e.g., <your-subscription-id>)"
    echo "  --production-tenant <tenant-id>            (e.g., <your-tenant-id>)"
    echo ""
    echo "Example:"
    echo "  ./run-migration-docker-auth.sh --use-api \\"
    echo "    --source-tenant <your-source-tenant-id> \\"
    echo "    --production-resource <your-resource-name> \\"
    echo "    --production-subscription <your-subscription-id> \\"
    echo "    --production-tenant <your-tenant-id> \\"
    echo "    asst_xxxxxxxxxxxxxxxxxxxxxxxx"
    exit 1
fi
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    if [ $EXIT_CODE -eq 1 ]; then
        ...
        echo "   • Reading assistants/agents : Foundry User on the resource (minimum for Foundry)"
        echo "   • Creating/writing agents   : Foundry User on the resource"
        echo "   • Uploading/downloading files: Foundry User on the resource"
