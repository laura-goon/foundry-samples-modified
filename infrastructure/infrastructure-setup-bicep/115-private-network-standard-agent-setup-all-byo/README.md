...
### Option 1: All-in-One Deployment (Default)
Deploy all resources (VNet, Storage, Cosmos DB, AI Search, and AI Foundry) in a single deployment using `main.bicep`.

### Option 2: Bring Your Own (BYO) Resources
First deploy prerequisite resources (VNet, Storage, Cosmos DB, AI Search) using `prerequisites.bicep`, then deploy AI Foundry infrastructure using `main.bicep` with the existing resource IDs.

**Benefits of BYO approach:**
- Greater control over resource lifecycle
- Ability to reuse existing infrastructure
- Separate management of foundational vs. application resources
- Easier to update AI Foundry components without affecting foundational resources

See [Deployment Instructions](#deployment-instructions) below for detailed steps for each option.
...
Deploy all resources in a single deployment using `main.bicep`:
...
### Option B: Bring Your Own (BYO) Resources Deployment

This approach provides greater control by separating prerequisite resources from AI Foundry infrastructure.

#### Step 1: Deploy Prerequisites

First, deploy the foundational resources (VNet, Storage, Cosmos DB, AI Search):

```powershell
# Using PowerShell script (recommended)
.\deploy-prerequisites.ps1 -ResourceGroupName "rg-ai-foundry-prereqs" -Location "eastus2"
```
...
az group create --name rg-ai-foundry-prereqs --location eastus2

# Deploy prerequisites
az deployment group create \
  --resource-group rg-ai-foundry-prereqs \
  --template-file prerequisites.bicep \
  --parameters prerequisites.bicepparam \
  --name prereqs-deployment
```

#### Step 2: Extract Outputs

If using the PowerShell script, it will automatically create `main-with-prereqs.bicepparam` with the resource IDs. Otherwise, manually extract the outputs:

```bash
az deployment group show \
  --resource-group rg-ai-foundry-prereqs \
  --name prereqs-deployment \
  --query properties.outputs.mainBicepParamInputs.value
```

#### Step 3: Deploy AI Foundry Infrastructure

Use the generated parameter file (or create your own with the resource IDs):
...
#### BYO Prerequisites Files

- **prerequisites.bicep**: Creates VNet (with agent and PE subnets), Storage Account, Cosmos DB, and AI Search
- **prerequisites.bicepparam**: Parameter file for prerequisites deployment
- **deploy-prerequisites.ps1**: Automated PowerShell script that:
  - Deploys prerequisites
  - Extracts resource IDs
  - Generates `main-with-prereqs.bicepparam` file ready for main deployment

#### Customizing Prerequisites

Edit `prerequisites.bicepparam` to customize network configuration and resource naming:

```bicep
// Customize location
param location = 'eastus2'

// Customize base name for Storage, Cosmos DB, and AI Search
// Creates: {baseName}{uniqueSuffix}storage, {baseName}{uniqueSuffix}cosmosdb, {baseName}{uniqueSuffix}search
param baseName = 'mycompany-prod'  // Example: becomes 'mycompany-prodabcdstorage'

// Customize VNet configuration
param vnetName = 'vnet-prod-agents-001'         // Full control over VNet name
param vnetAddressPrefix = '10.0.0.0/16'          // Customize VNet CIDR

// Customize subnet names and ranges
param agentSubnetName = 'snet-containerApps'     // Full control over agent subnet name
param agentSubnetPrefix = '10.0.1.0/24'          // Customize agent subnet

param peSubnetName = 'snet-privateendpoints'     // Full control over PE subnet name
param peSubnetPrefix = '10.0.2.0/24'             // Customize PE subnet
```

**Naming Convention Notes:**
- **Network resources** (VNet, subnets): Use exact names you specify
- **Data resources** (Storage, Cosmos DB, AI Search): Names are derived from `baseName` + unique 4-character suffix
  - The suffix ensures uniqueness across Azure subscriptions
  - Storage: `{baseName}{suffix}storage` (e.g., `mycompanyprodabcdstorage`)
  - Cosmos DB: `{baseName}{suffix}cosmosdb` (e.g., `mycompanyprodabcdcosmosdb`)
  - AI Search: `{baseName}{suffix}search` (e.g., `mycompanyprodabcdsearch`)
